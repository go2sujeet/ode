...
export type QwenInspectorToolState = SessionTool & {
  inputBuffer?: string;
};

export type QwenStreamStateMaps = {
  textByIndex: Map<number, string>;
  thinkingByIndex: Map<number, string>;
  toolByIndex: Map<number, QwenInspectorToolState>;
  toolById: Map<string, QwenInspectorToolState>;
};
...
export function applyQwenRecordToState(
  ...
  if (record.type === "assistant") {
    ...
    for (const block of blocks) {
      ...
      const tool: QwenInspectorToolState = {
        id: toolId,
        name: toolName,
  ...
  if (record.type === "user") {
    ...
    for (const block of blocks) {
      ...
      const updated: QwenInspectorToolState = {
        ...existing,
