<script lang="ts">
    ...
    function getToolDetails(tool: any): ToolDetails {
        ...
        if (name === "grep" || name === "ripgrep" || name === "rg") {
            ...
            if (details && (offset !== undefined || limit !== undefined)) {
                ...
                const rangeLabel = [offsetLabel, limitLabel]
                    .filter(Boolean)
                    .join(", ");
    ...
    function computeDiff(
        ...
        for (let i = 0; i < contextAfter; i++) {
            ...
            right.push({
                type: "context",
                text: " " + newLines[newSuffixStart + i],
            });
        }

        if (left.length > maxLines) {
            return {
                left: left.slice(0, maxLines),
                right: right.slice(0, maxLines),
            };
        }
    ...
    $: toolItems = state.tools.length > 5 ? state.tools.slice(-5) : state.tools;
    $: toolItemsLabel =
        state.tools.length > 5
            ? `Tool execution (Last 5 items in ${state.tools.length})`
            : "Tool execution";
</script>
...
<div class="slack-message">
    <div class="status-section">
        <div class="status-line">
            <span class="status-text">{state.sessionTitle}</span>
            <span class="elapsed-time"
                >{formatElapsedTime(state.startedAt)}</span
            >
        </div>
    </div>
    ...
    {#if state.tools.length > 0}
        <div class="tools-section">
            <div class="section-title">{toolItemsLabel}</div>
            {#each toolItems as tool}
                ...
                <div class="tool-item">
                    ...
                    {#if details.details}
                        <span
                            class="tool-details"
                            class:wrap-details={details.name.toLowerCase() ===
                                "read"}
                        >
                            {details.details}
                        </span>
                ...
                {#if details.diff}
                    {@const diffResult = computeDiff(
                        details.diff.oldString,
                        details.diff.newString,
                        10,
                    )}
                    <div class="tool-diff">
                        <div class="diff-side">
